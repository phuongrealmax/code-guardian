...
interface CCGModules {
  ...
  rag: RAGModule;
  codeOptimizer: CodeOptimizerService;  // NEW: Code Optimizer v3.1
}
...
export async function createCCGServer(): Promise<Server> {
  ...
  const modules: CCGModules = {
    ...
    thinking: new ThinkingModule((config.modules as any).thinking || DEFAULT_THINKING_CONFIG, eventBus, logger, projectRoot),
    autoAgent: new AutoAgentModule((config.modules as any).autoAgent || DEFAULT_AUTO_AGENT_CONFIG, eventBus, logger),
    // RAG Module v3.0 - Semantic codebase search
    rag: new RAGModule(eventBus, (config.modules as any).rag || DEFAULT_RAG_CONFIG),
    // NEW: Code Optimizer v3.1 - Repository analysis & refactoring
    codeOptimizer: new CodeOptimizerService(
      (config.modules as any).codeOptimizer || DEFAULT_CODE_OPTIMIZER_CONFIG,
      eventBus,
      logger,
      projectRoot
    ),
