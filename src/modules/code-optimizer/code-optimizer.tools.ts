...
export function createCodeOptimizerToolHandlers(service: CodeOptimizerService) {
  return {
    code_scan_repository: async (args: ScanRepositoryInput) => {
      try {
    ...
    code_metrics: async (args: MetricsInput) => {
      try {
    ...
    code_record_optimization: async (args: RecordOptimizationInput) => {
      try {
...
function formatMetricsResult(result: any): string {
  ...
  const sorted = [...result.files].sort((a: any, b: any) => b.complexityScore - a.complexityScore);
...
function formatHotspotsResult(result: any): string {
...
function formatPlanResult(result: any): string {
