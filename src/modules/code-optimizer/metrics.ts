...
import { getLanguageFromExtension, isSourceCodeFile } from './repo-scanner.js';
...
function analyzeFileContent(filePath: string, content: string): FileMetrics {
  ...
  // Branch keywords
  const branchKeywords: BranchKeywordsCount = {
    if: 0,
    elseIf: 0,
    switch: 0,
    case: 0,
  ...
  for (const line of lines) {
    ...
    if (
      ...
      if (/\bTODO\b/i.test(trimmedLine)) todoCount++;
      if (/\bFIXME\b/i.test(trimmedLine)) fixmeCount++;
      continue;
    }
...
function countBranchKeywords(line: string, counts: BranchKeywordsCount): void {
  // Match word boundaries to avoid false positives
  const patterns: Array<{ pattern: RegExp; key: keyof BranchKeywordsCount }> = [
 ...
 */
export function quickComplexityCheck(content: string): {
  isComplex: boolean;
  reasons: string[];
} {
  const lines = content.split('\n');
