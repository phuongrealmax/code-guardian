...
export function buildRefactorPlan(input: RefactorPlanInput): RefactorPlanOutput {
  const {
    hotspots,
    goal,
    maxStepsPerFile = 5,
  ...
  const phases: RefactorPhase[] = ['analysis', 'plan', 'impl', 'review'];
...
function generateStepsForFile(
  hotspot: RefactorPlanInput['hotspots'][0],
  goal: RefactorGoal,
  maxSteps: number,
  constraints: string[]
): RefactorStep[] {
...
function addReadabilitySteps(steps: RefactorStep[], reason: string, maxSteps: number): void {
...
function addPerformanceSteps(steps: RefactorStep[], reason: string, maxSteps: number): void {
  steps.push(createStep({
...
function addArchitectureSteps(steps: RefactorStep[], reason: string, maxSteps: number): void {
  steps.push(createStep({
...
function addTestingSteps(steps: RefactorStep[], reason: string, maxSteps: number): void {
  steps.push(createStep({
...
function addMixedSteps(steps: RefactorStep[], reason: string, maxSteps: number): void {
...
function createStep(params: Omit<RefactorStep, 'id'>): RefactorStep {
  return {
...
function getGoalDescription(goal: RefactorGoal, reason: string): string {
  const goalDescriptions: Record<RefactorGoal, string> = {
...
function estimateEffort(steps: RefactorStep[]): 'small' | 'medium' | 'large' {
  const riskWeights = { low: 1, medium: 2, high: 4 };
  const totalWeight = steps.reduce((sum, s) => sum + riskWeights[s.risk], 0);

  if (totalWeight <= 4) return 'small';
  if (totalWeight <= 10) return 'medium';
  return 'large';
}

function estimateTotalEffort(plans: FilePlan[]): string {
...
export function getStepsForPhase(
  plan: RefactorPlanOutput,
  phase: RefactorPhase
): Array<{ file: string; step: RefactorStep }> {
  const result: Array<{ file: string; step: RefactorStep }> = [];
