...
export function selectHotspots(input: HotspotsInput): HotspotsOutput {
  const {
    metrics,
    maxResults = 20,
    strategy = 'mixed',
  ...
  // Add rank
  const rankedHotspots: Hotspot[] = topHotspots.map((h, index) => ({
    ...h,
    rank: index + 1,
  }));
...
function calculateHotspotScore(
  ...
  switch (strategy) {
    ...
    case 'complexity':
      ...
      return (
        normalizeScore(complexity, 10, 80) * 50 +
...
function identifyReasons(m: HotspotsInput['metrics'][0]): string[] {
...
function determineSuggestedGoal(
  m: HotspotsInput['metrics'][0],
  reasons: string[]
): SuggestedGoal {
  const complexity = m.complexityScore ?? m.branchScore;
 ...
 */
export function groupHotspotsByGoal(
  hotspots: Hotspot[]
): Record<SuggestedGoal, Hotspot[]> {
  const groups: Record<SuggestedGoal, Hotspot[]> = {
    refactor: [],
    'add-tests': [],
    'split-module': [],
    simplify: [],
 ...
 */
export function getPriorityOrder(hotspots: Hotspot[]): string[] {
