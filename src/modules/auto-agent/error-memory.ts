...
export class ErrorMemory {
  private config: AutoAgentModuleConfig['errorMemory'];
  private logger: Logger;
  private eventBus: EventBus;

  // In-memory storage (would be persisted in production)
  private errors: Map<string, ErrorMemoryEntry> = new Map();
  ...
  constructor(
    config: AutoAgentModuleConfig['errorMemory'],
    logger: Logger,
    eventBus: EventBus
  ) {
    this.config = config;
   ...
   */
  async store(params: StoreErrorParams): Promise<ErrorMemoryEntry> {
   ...
   */
  async recall(params: RecallErrorsParams): Promise<RecallErrorsResult> {
    const { error, tags, limit = 5, minSimilarity = 0.3 } = params;

    let matches: ErrorMemoryEntry[] = [];
    ...
    let suggestedFix: FixAction | undefined;
    let confidence = 0;
   ...
   */
  private findSimilar(error: ErrorInfo): ErrorMemoryEntry | null {
    let bestMatch: ErrorMemoryEntry | null = null;
   ...
   */
  private findSimilarErrors(error: ErrorInfo, minSimilarity: number): ErrorMemoryEntry[] {
    const results: ErrorMemoryEntry[] = [];
   ...
   */
  private calculateSimilarity(a: ErrorInfo, b: ErrorInfo): number {
    let score = 0;
    let weights = 0;
   ...
   */
  private updatePatterns(error: ErrorInfo, fix: FixAction, success: boolean): void {
   ...
   */
  private findMatchingPattern(error: ErrorInfo): ErrorPattern | null {
  ...
  private evictOldest(): void {
    let oldest: ErrorMemoryEntry | null = null;
