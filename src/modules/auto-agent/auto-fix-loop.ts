...
const ERROR_FIX_PATTERNS: Array<{
  pattern: RegExp;
  errorType: string;
  fixGenerator: (match: RegExpMatchArray, error: ErrorInfo) => FixAction;
}> = [
  // TypeScript errors
  {
...
export class AutoFixLoop {
  private config: AutoAgentModuleConfig['fixLoop'];
  private logger: Logger;
  private eventBus: EventBus;
  private status: FixLoopStatus = 'idle';
  private currentAttempts: FixAttempt[] = [];
  ...
  constructor(
    config: AutoAgentModuleConfig['fixLoop'],
    logger: Logger,
    eventBus: EventBus
  ) {
    this.config = config;
   ...
   */
  private generateFix(error: ErrorInfo): FixAction | null {
  ...
  private async applyFix(
    fix: FixAction,
    context: StartFixLoopParams['context']
  ): Promise<boolean> {
   ...
   * Rollback changes
   */
  private async rollback(context: StartFixLoopParams['context']): Promise<boolean> {
