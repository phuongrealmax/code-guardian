...
export class ToolRouter {
  private config: AutoAgentModuleConfig['router'];
  private logger: Logger;
  private eventBus: EventBus;
  private rules: ToolRoutingRule[];
  ...
  constructor(
    config: AutoAgentModuleConfig['router'],
    logger: Logger,
    eventBus: EventBus
  ) {
    this.config = config;
  ...
  private matchRule(
    ...
    action: string,
    context?: RouteToolParams['context']
  ): boolean {
   ...
   */
  private matchKeywords(pattern: string, action: string): boolean {
   ...
   */
  private calculateConfidence(matchedRules: number, suggestedTools: number): number {
    if (matchedRules === 0) return 0;
    if (matchedRules >= 3 && suggestedTools >= 2) return 0.9;
    if (matchedRules >= 2) return 0.7;
    if (matchedRules >= 1) return 0.5;
    return 0.3;
  }
}
