...
export class ThinkingService {
  private models: Map<string, ThinkingModel> = new Map();
  ...
  async initialize(): Promise<void> {
    ...
    this.logger.info(
      `Thinking module initialized: ${this.models.size} models, ${this.workflows.size} workflows, ${this.snippets.size} snippets`
    );
  }
  ...
  getThinkingModel(modelName: ThinkingModelType): ThinkingModel | null {
    ...
    if (model) {
      this.eventBus.emit({
        type: 'thinking:model_accessed' as any,
        timestamp: new Date(),
        data: { modelName },
  ...
  getWorkflow(workflowName: WorkflowType): Workflow | null {
    ...
    if (workflow) {
      this.eventBus.emit({
        type: 'thinking:workflow_accessed' as any,
        timestamp: new Date(),
        data: { workflowName },
        source: 'ThinkingService',
      });
   ...
   */
  async saveSnippet(params: SaveSnippetParams): Promise<CodeSnippet> {
    ...
    const categorySnippets = Array.from(this.snippets.values()).filter(
    ...
    if (categorySnippets.length >= maxPerCategory) {
      ...
      const oldest = categorySnippets.sort(
    ...
    this.eventBus.emit({
      type: 'thinking:snippet_saved' as any,
      timestamp: new Date(),
   ...
   */
  getStyleReference(category: string, limit: number = 3): StyleReferenceResult {
    this.lastAccessed = new Date();
  ...
  private async loadCustomModels(): Promise<void> {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const model = JSON.parse(content) as ThinkingModel;
        if (model.id) {
  ...
  private async persistSnippet(snippet: CodeSnippet): Promise<void> {
    if (!existsSync(this.snippetsDir)) {
      mkdirSync(this.snippetsDir, { recursive: true });
    }
