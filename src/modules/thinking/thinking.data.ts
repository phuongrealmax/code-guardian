...
export const DEFAULT_THINKING_MODELS: Record<ThinkingModelType, ThinkingModel> = {
  'chain-of-thought': {
    id: 'chain-of-thought',
...
export const DEFAULT_WORKFLOWS: Record<WorkflowType, Workflow> = {
  'pre-commit': {
    id: 'pre-commit',
 ...
 */
export function getThinkingModelNames(): ThinkingModelType[] {
  return Object.keys(DEFAULT_THINKING_MODELS) as ThinkingModelType[];
}

/**
 * Get all workflow names
 */
export function getWorkflowNames(): WorkflowType[] {
  return Object.keys(DEFAULT_WORKFLOWS) as WorkflowType[];
}

/**
 * Suggest thinking model based on task description
 */
export function suggestThinkingModel(taskDescription: string): ThinkingModelType {
 ...
 */
export function suggestWorkflow(taskDescription: string): WorkflowType | null {
  const desc = taskDescription.toLowerCase();
  ...
  for (const [workflowId, workflow] of Object.entries(DEFAULT_WORKFLOWS)) {
    for (const keyword of workflow.triggerKeywords) {
      if (desc.includes(keyword.toLowerCase())) {
        return workflowId as WorkflowType;
      }
    }
  }
