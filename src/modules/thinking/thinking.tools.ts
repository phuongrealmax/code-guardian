...
export function createThinkingToolHandlers(service: ThinkingService) {
  return {
    ...
    thinking_get_workflow: (args: GetWorkflowParams) => {
      const workflow = service.getWorkflow(args.workflowName as WorkflowType);
    ...
    thinking_suggest_workflow: (args: { taskDescription: string }) => {
    ...
    // Code Snippets
    thinking_save_snippet: async (args: SaveSnippetParams) => {
    ...
    thinking_get_style: (args: GetStyleReferenceParams) => {
