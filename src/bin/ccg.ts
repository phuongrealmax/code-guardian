...
program
  ...
  .option('-j, --json', 'Output as JSON')
  .action(async (options: { summary?: boolean; trend?: boolean; count?: string; json?: boolean }) => {
    const cwd = process.cwd();
    const ccgDir = join(cwd, '.ccg');
    ...
    try {
      ...
      if (options.trend) {
        ...
        // Default: show latest session details
...
function getGradeLetter(index: number): string {
