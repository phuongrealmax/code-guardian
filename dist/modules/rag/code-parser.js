 ...
 */
export function parseSourceFile(filePath, content, language) {
    const chunks = [];
    const errors = [];
 ...
 */
function extractFunctions(filePath, content, lines, language, pattern, imports) {
    const chunks = [];
    let match;
    while ((match = pattern.exec(content)) !== null) {
        const name = match[1];
 ...
 */
function extractClasses(filePath, content, lines, language, pattern, imports) {
    const chunks = [];
    let match;
    while ((match = pattern.exec(content)) !== null) {
        const name = match[1];
 ...
 */
function extractInterfaces(filePath, content, lines, language, pattern) {
    const chunks = [];
    let match;
    while ((match = pattern.exec(content)) !== null) {
        const name = match[1];
 ...
 * Extract type aliases
 */
function extractTypes(filePath, content, lines, language, pattern) {
    const chunks = [];
    let match;
    while ((match = pattern.exec(content)) !== null) {
        const name = match[1];
 ...
 */
function createModuleChunk(filePath, content, lines, language, imports) {
 ...
 */
function extractBlock(content, startIndex, language) {
 ...
 */
function findStatementEnd(content, startIndex, lines) {
    const startLine = getLineNumber(content, startIndex);
    let depth = 0;
 ...
 */
function generateChunkId(filePath, type, name, line) {
