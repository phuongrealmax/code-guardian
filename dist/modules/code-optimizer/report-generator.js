 ...
 */
export function generateReport(input, projectRoot) {
    const sections = [];
    ...
    const outputPath = input.outputPath || `docs/reports/optimization-${date}-${input.sessionId}.md`;
...
//                      SECTION GENERATORS
// ═══════════════════════════════════════════════════════════════
function generateHeader(sessionId, repoName, date, strategy) {
...
function generateOverviewSection(scan) {
    return `## Overview
...
function generateMetricsSection(before, after) {
    if (!before && !after) {
    ...
    if (before && after) {
        ...
        const complexityChange = calculatePercentChange(before.aggregate.avgComplexityScore, after.aggregate.avgComplexityScore);
        const linesChange = calculatePercentChange(before.aggregate.totalLines, after.aggregate.totalLines);
...
function generateFilesChangedSection(filesChanged, descriptions) {
...
function generateRefactorPlanSection(plan) {
...
function generateTestResultsSection(input) {
    const statusEmoji = {
        passed: '✅',
        failed: '❌',
        partial: '⚠️',
...
function generateNextStepsSection(hotspots) {
