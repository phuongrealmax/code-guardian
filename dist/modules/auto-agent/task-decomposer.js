...
export class TaskDecomposer {
    ...
    analyzeComplexity(params) {
        ...
        const estimatedSubtasks = Math.min(Math.max(Math.round(normalizedScore / 2), 2), this.config.maxSubtasks);
        return {
    ...
    async decompose(params) {
        ...
        const subtasks = template.map((t, index) => ({
            ...t,
            id: uuid(),
            order: index + 1,
